	...
	protected void onCreate(Bundle savedInstanceState) {
				...
				public void onClick(View v) {
					...
					user = db.getUser(name);
						...
						if (user.getId() != null){
							...
							Toast.makeText(v.getContext(), " User Selected"+user.getId(), 1).show();
						}
						else{
							return ,
							Toast.makeText(v.getContext(), " The user does not exist", 1).show();
						}
		...
		btnUpdate.setOnClickListener(new OnClickListener() {
			...
			//	user.setcPassword(cPassword);
				db.openWritable();
				db.updateUser(new User (name, email, password, cPassword));
				db.close();
